...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    fun compartirTexto() {
        val intent: Intent = Intent().apply {
            ...
            putExtra(Intent.EXTRA_TEXT, "Descarga la aplicación desde: https://play.google.com/id=myapp")
            type = "text/plain"
        }
        val shareIntent = Intent.createChooser(intent, "Compartir texto con...")
        if(intent.resolveActivity(packageManager) != null) {
        ...
        when(requestCode) {
            ...
            FOTO_CAMARA -> {
                if(resultCode == Activity.RESULT_OK) {
                    ...
                    ibImagenCamara.setImageBitmap(image)
                } else {
                    Toast.makeText(this, "Se cancelo la captura de la camara", Toast.LENGTH_LONG).show()
                }
            }
            FOTO_GALERIA -> {
                if(resultCode == Activity.RESULT_OK) {
                    ...
                    Toast.makeText(this, "Se cancelo la obtención de Galeria", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
